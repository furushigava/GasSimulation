...
def update_energy_conservation_graphs(figure, canvas, data):
    ...
    # Если начальная энергия не задана, берем первое значение
    if initial_energy is None or initial_energy == 0:
        initial_energy = kinetic_energy[0] if len(kinetic_energy) > 0 else 1.0
    
    # 1. Кинетическая энергия от времени
    ax1 = figure.add_subplot(231)
    ax1.plot(time, kinetic_energy, 'b-', linewidth=1.5, label='E(t)')
    ax1.axhline(y=initial_energy, color='r', linestyle='--', linewidth=1.5, label=f'E₀ = {initial_energy:.2f}')
    ax1.set_xlabel('Время')
    ax1.set_ylabel('Кинетическая энергия')
    ax1.set_title('Кинетическая энергия системы')
    ax1.legend(loc='best')
    ...
    if initial_energy != 0:
        relative_error = (kinetic_energy - initial_energy) / initial_energy * 100
    ...
    # 3. Абсолютное изменение энергии
    ...
    if len(kinetic_energy) > 10:
        deviations = kinetic_energy - initial_energy
    ...
    if len(kinetic_energy) > 10:
        window_size = min(20, len(kinetic_energy) // 5)
        if window_size > 0:
            moving_avg = np.convolve(kinetic_energy, np.ones(window_size)/window_size, mode='valid')
            time_avg = time[:len(moving_avg)]
            ax5.plot(time, kinetic_energy, 'b-', alpha=0.3, linewidth=1, label='E(t)')
    ...
    if len(kinetic_energy) > 0:
        current_energy = kinetic_energy[-1]
        mean_energy = np.mean(kinetic_energy)
        std_energy = np.std(kinetic_energy)
        max_deviation = np.max(np.abs(kinetic_energy - initial_energy))
        ...
        status_color = 'green' if is_isolated else 'orange'
...
Текущая энергия E(t): {current_energy:.4f}
